...
use defmt_setup::*;
use hinge::motor::{pwm_init, Motor, Driver};
use hinge::motor::{encoder, interrupts};
use hinge::{Controls, Hinge};
...
async fn test(controls: &'static Controls) {
    loop {
        controls.set_target(-65);
        Timer::after(Duration::from_millis(10000)).await;
        controls.set_target(-10);
        Timer::after(Duration::from_millis(10000)).await;
        controls.set_target(-30);
        Timer::after(Duration::from_millis(10000)).await;
    }
}
